...
static const char *fonts[] = {"Weather Icons:size=15",
                              "Apple Color Emoji:size=15",
                              "MesloLGS Nerd Font:size=15"};
...
static const char *rofiEmojiCmd[] = {"rofi",  "-modi",        "emoji",  "-show",
                                     "emoji", "-kb-custom-1", "Ctrl+C", NULL};
...
void screenshot(const Arg *arg) {
  system(" scrot  ~/screenshots/$(date +\"%F_%T\").jpg && dunstify -t 1000 "
         "\"Screenshot saved to: ~/screenshots\"");
}
void selectedscreenshot(const Arg *arg) {
  system(" scrot -s ~/screenshots/$(date +\"%F_%T\").jpg && dunstify -t 1000 "
         "\"Screenshot saved to: ~/screenshots\"");
}

// Audio
void lowerVolume(const Arg *arg) {
  system("pactl set-sink-volume @DEFAULT_SINK@ -10%");
}
void raiseVolume(const Arg *arg) {
  system("pactl set-sink-volume @DEFAULT_SINK@ +10%");
}
void audioMute(const Arg *arg) {
  system("pactl set-sink-mute @DEFAULT_SINK@ toggle");
}
void audioMicMute(const Arg *arg) {
